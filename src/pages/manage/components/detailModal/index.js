...
export default connect(models => ({
  ...
  const { visible, setVisible, data, dispatch, manage } = props;
  const [list, setList] = useState([]);

  const toDetail = record => {
    if (record.step === 1) {
      router.push(`/manage/upload?batchName=${record.batchName}`);
    } else {
      router.push(`/manage/check?batchName=${record.batchName}`);
    }
  };
  const cancel = record => {
    dispatch({
    ...
    setList(setList(Object.keys(manage).map(i => manage[i])));
  };
  const getProgress = record => {
    ...
    // if (record.step === 1) {
    //   if (record.load.filter(i => i === false).length === 2) {
    //     return '已完成';
    //   }
