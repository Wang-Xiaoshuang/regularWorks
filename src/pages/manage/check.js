...
import { toSave, uploadZip } from './services';
...
export default connect(models => ({
  ...
  const { table, loadList } = manage[batchName] || {};
  const [visible, setVisible] = useState(false);
  const [list, setList] = useState(table);

  useEffect(() => {
    if (!manage || !manage[batchName]) {
  ...
  const renderProgress = () => {
    ...
    const batchData = manage[batchName];
        ...
        {load[1] === 'loading' ? (
          <div className={styles.progress}>
            <Progress
              size="samll"
              strokeWidth={4}
              percent={manage[batchName].continueZipProgress}
            />
          </div>
        ) : null}
      </div>
  ...
  return (
    <div>
      ...
      <div className="duplicate_page">
        ...
        {batchName && manage[batchName] && manage[batchName].continueZipProgress > 0
          ? renderProgress()
          : null}
        ...
        <div className={styles.content}>
          ...
          <div className={styles.btns}>
            <Space>
              ...
              <Button onClick={() => router.push('/manage/list')}>后台解析上传</Button>
              <Button
                onClick={toSave1}
                disabled={
